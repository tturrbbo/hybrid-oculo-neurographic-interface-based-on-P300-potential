...
def _project_root() -> Path:
    # file: eyegaze/app/hybrid_gaze_p300_runner.py
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Гибридный запуск: eyegaze tiles + P300 Analyzer в двух окнах."
    )
    ...
    p.add_argument(
        "--keep-p300",
        action="store_true",
        help="не закрывать P300 Analyzer при выходе из eyegaze",
    )
    return p.parse_args()
...
def main():
    ...
    if p300_proc is not None:
        print("[hybrid] P300 Analyzer запущен отдельным окном.")
        print("[hybrid] В P300 нажми: подключиться к LSL -> начать анализ.")
        time.sleep(1.0)
    ...
    print("[hybrid] Запуск eyegaze:", " ".join(cmd))
